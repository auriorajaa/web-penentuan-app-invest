document.addEventListener('DOMContentLoaded', () => {
   ...
   function showResults() {
      ...
      questions.forEach((question, index) => {
         ...
         if (question.isCost) {
            // Normalisasi terbalik untuk kriteria biaya
                              ...
                              <tbody class="text-gray-500">
                                  ${questions.map((question, index) => `
                                      <tr>
                                          ...
                                          ${Object.keys(totalScores).map(app => {
            const score = question.isCost ? ((question.scores[app] - maxScore) / maxScore) : question.scores[app];
            return `<td class="py-2 px-4 border-b">${(score * userWeights[index]).toFixed(2)}</td>`;
         }).join('')}
                                      </tr>
                                  `).join('')}
                              </tbody>
